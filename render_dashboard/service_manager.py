...
def handle_service_management(args: list[str], config_path: Optional[Path] = None) -> int:
    ...
    if len(args) == 0:
        ...
        print()
        print("Manage Render services in your config file. Services you add can be")
        print("quickly accessed with: rdash <alias> logs|events|status|settings")
        print()
        print("Commands:")
        print("  add <name>     - Search and add a service by name or ID")
        print("  list           - List configured services")
        print("  remove <alias> - Remove a service from config")
        print()
        print("Examples:")
        print("  rdash service add chat       # Search for and add 'chat' service")
        print("  rdash service list           # Show all configured services")
        print("  rdash service remove chat    # Remove service by alias")
        print()
        print("For detailed help on a command, run it without arguments:")
        print("  rdash service add")
        return 1
    ...
    if command == "add":
        if len(args) < 2:
            ...
            print()
            print("Description:")
            print("  Add a Render service to your config for easy CLI access.")
            print()
            print("Two Ways to Add a Service:")
            print()
            print("  1. By Name (Recommended)")
            print("     Searches your Render account for matching services")
            print("     Example: rdash service add chat")
            print()
            print("  2. By Service ID")
            print("     Directly add a service if you know its ID")
            print("     Example: rdash service add srv-xxxxxxxxxxxxx")
            print()
            print("Finding Your Service ID:")
            print("  1. Go to https://dashboard.render.com")
            print("  2. Click on a service")
            print("  3. Copy the 'srv-xxxxx' from the URL:")
            print("     https://dashboard.render.com/web/srv-xxxxxxxxxxxxx")
            print()
            print("What You'll Be Asked:")
            print("  - Primary alias (shortcut name for CLI)")
            print("  - Additional aliases (optional, comma-separated)")
            print()
            print("Config File Location (checked in order):")
            print("  1. --config flag: rdash --config ~/my.yaml service add chat")
            print("  2. RENDER_DASHBOARD_CONFIG environment variable")
            print("  3. ./config.yaml (current directory)")
            print("  4. ~/.config/render-dashboard/config.yaml")
            return 1
